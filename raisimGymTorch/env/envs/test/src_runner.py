...
cfg['environment']["harsh_termination"] = True
...
for grid_size in [2.5, 3., 4., 5.]:
    ...
    assert num_total_test_case == (collision_success + collision_fail + no_collision_success + no_collision_fail)

    collision_safe = collision_success / (collision_success + collision_fail)
    no_collision_safe = no_collision_success / (no_collision_success + no_collision_fail)

    # Save summarized result
    final_result = defaultdict(dict)
    final_result["Collision"]["success"] = collision_success
    final_result["Collision"]["fail"] = collision_fail
    final_result["Collision"]["safe"] = collision_safe
    final_result["No collision"]["success"] = no_collision_success
    final_result["No collision"]["fail"] = no_collision_fail
    final_result["No collision"]["safe"] = no_collision_safe
